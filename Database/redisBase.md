...
- 定时删除:redis会将每个设置了过期时间的key放入一个独立的字典中,遍历key发现过期的就删除
  - 贪心策略:不会全部遍历key
  - 从字典中随机挑出20个key
  - 删除这其中过期的key
  - 如果过期的key超过1/4,重复再随机20个key;防止过度循环,线程卡死,会设置默认扫描时间上限25ms;
...
- Select
- Dbsize
- Flushdb / Flushall / flushall async
