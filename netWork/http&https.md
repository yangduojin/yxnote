...
逻辑端口，是TCP/IP协议的端口，也常叫网络端口。

0~1023是公认端口，比如80就默认是http的端口、443就是https的端口.当自定义的时候可以不用443,只要服务端许可

### 使用443的端口的同时，80端口还会用吗？

这就是涉及到https的本质了。https本质是http协议上加了一个安全层。安全层做了一些处理，http要做的事情一点也没有减少。

### 安全层原理

靠加密,https协议是用非对称加密沟通一个对称秘钥。真正的信息就用对称秘钥来加解密了。

### CA证书
	
网络抓包吧的原理和黑客截获信息差不多,抓包者充当代理的原理。CA证书大家都很认可的组织，它们可以颁发公钥。
