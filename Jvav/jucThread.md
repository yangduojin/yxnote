...
2. 当提交的任务>核心线程数 没有处理的任务会在队列中等待
3. 如果等待队列满了 当提交的任务>(核心线程数+队列大小) 创建非核心线程去处理任务
4. 如果等待队列满了 当提交的任务>(max线程数+队列大小)  采用拒绝策略
...
1. 分布式协调系统如zookeeper、etcd，客户端都是使用一个单一长连接。记得之前使用k8s客户端(核心是etcd)时候，需要自己实现close()接口，我在close接口里判断如果连接断了，就重新连接。zookeeper的客户端实现也是大同小异。
...
2. 但是像数据库连接、MQ连接这种，更常见的实现方式是连接池。
...
3. 建议常常做业务开发、使用数据库连接池的朋友结合下面常用参数参考看几遍DruidDataSource的源码。以便更好的了解参数的使用。
![德鲁伊数据库连接参数](./img/DruidDataSourceArgs.png)
