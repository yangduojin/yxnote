...
- [Synchronized](#synchronized)
  ...
  - [3 同步概念](#3-同步概念)
    ...
    - [3.2 对象头中Mark Word与线程中Lock Record](#32-对象头中mark-word与线程中lock-record)
    - [3.3 监视器（Monitor）](#33-监视器monitor)
  - [4 锁的优化](#4-锁的优化)
    - [4.1 自旋锁](#41-自旋锁)
    - [4.2 适应性自旋锁](#42-适应性自旋锁)
    - [4.3 锁消除](#43-锁消除)
    - [4.4 锁粗化](#44-锁粗化)
    - [4.5 偏向锁](#45-偏向锁)
    - [4.6 轻量级锁](#46-轻量级锁)
    - [4.7 重量级锁](#47-重量级锁)
    - [4.8 重量级锁、轻量级锁和偏向锁之间转换](#48-重量级锁轻量级锁和偏向锁之间转换)
...
![反编译后结果](/Jvav/img/syncDetail2.png)
...
![对象头分布](/Jvav/img/syncDetail3.png)
...
Synchronized用的锁就是存在Java对象头里的，那么什么是Java对象头呢？Hotspot虚拟机的对象头主要包括两部分数据：**Mark Word（标记字段）、**Class Pointer（类型指针）。其中Class Pointer是对象指向它的类元数据的指针，虚拟机通过这个指针来确定这个对象是哪个类的实例，Mark Word用于存储对象自身的运行时数据，它是实现轻量级锁和偏向锁的关键。 Java对象头具体结构描述如下：

![Java对象头具体结构](/Jvav/img/syncDetail4.png)
...
![Mark Word](/Jvav/img/syncDetail5.png)
...
![Mark Word](/Jvav/img/syncDetail6.png)

Mark Word可能存储4种数据

在64位虚拟机下，Mark Word是64bit大小的，其存储结构如下：

![Mark Word存储结构](/Jvav/img/syncDetail7.png)
...
![64位Mark Word存储结构](/Jvav/img/syncDetail8.png)
...
![notifyAll](/Jvav/img/syncDetail9.png)
...
在JDK 1.6 中默认是开启偏向锁和轻量级锁的，可以通过-XX:-UseBiasedLocking来禁用偏向锁。
...
![SMP的结构](/Jvav/img/syncDetail10.png)
...
> 上面提到Cache一致性，其实是有协议支持的，现在通用的协议是MESI（最早由Intel开始支持），具体参考：[http://en.wikipedia.org/wiki/MESI\_protocol](http://en.wikipedia.org/wiki/MESI_protocol)。
...
![syncDetail11](/Jvav/img/syncDetail11.png)
...
1. 在线程进入同步块时，如果同步对象锁状态为无锁状态（锁标志位为“01”状态，是否为偏向锁为“0”），虚拟机首先将在当前线程的栈帧中建立一个名为锁记录（Lock Record）的空间，用于存储锁对象目前的Mark Word的拷贝，官方称之为 Displaced Mark Word。此时线程堆栈与对象头的状态如下图所示：
    ...
    ![线程堆栈与对象头的状态](/Jvav/img/syncDetail12.png)
...
4. 如果这个更新动作成功了，那么当前线程就拥有了该对象的锁，并且对象Mark Word的锁标志位设置为“00”，即表示此对象处于轻量级锁定状态，此时线程堆栈与对象头的状态如下图所示：
    ...
    ![线程堆栈与对象头的状态](/Jvav/img/syncDetail13.png)
...
![锁转换](/Jvav/img/syncDetail15.png)

重量级锁、轻量级锁和偏向锁之间转换

![锁转换](/Jvav/img/syncDetail16.png)
