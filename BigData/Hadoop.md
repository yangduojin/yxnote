...
- **文件块**
- defaultBlockSize2.x/3.x: 128M, 1.x: 64M, 可以通过dfs.blocksize 来配置大小
- 寻址时间为传输时间的1%时,则为最佳状态(专家)
- 文件块太小会增加寻址时间,一直在找开始位置, 文件快太大从磁盘传输数据的时间会明显大于定位这个块开始位置所需的时间,导致程序在处理这块数据时会非常慢.
...
|名称|章节|内容|
|--|--|--|
|MapReduce|概述|定义/ 优势劣势/ WordCount案例|
||序列化||
||核心框架原理(重要)|输入的数据InputFormat|
|||Shuffle|
|||输出数据OutputFormat|
|||Join|
|||ETL|
|||总结|
||压缩|有哪些压缩算法,特点,在生产环境怎么用|
||常见的问题及解决方案||
